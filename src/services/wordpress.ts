...
export const getPosts = async (postType: string, params: any = {}) => {
	...
	const res = await fetch(
		`${process.env.NEXT_PUBLIC_WP_URL}/wp-json/wp/v2/${postType}/${
			params?.id ? params?.id : ''
		}?order=${params.order || 'asc'}&orderby=${
			params.orderby || 'id'
		}&per_page=${params.per_page || 100}&page=${page}`
	);
...
export const findPost = async (
	...
	while (true) {
		...
		const { items, pages } = await getPosts(postType, payload);
		const found = (Array.isArray(items) ? items : [items]).find((item: any) => {
			if (type === 'simple') return item.id === filterValue;
